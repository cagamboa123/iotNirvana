...
function gceInstanceCreation() {
  ...
  var url = URL_PREFIX + '/rest/gce/instance';
...
function gceInstanceRunningList() {
  $.ajax({
    type: 'GET',
    url: URL_PREFIX + '/rest/gce/instance',
    async: true,
...
function gceInstanceRunningDelete() {
  ...
  xhr = new XMLHttpRequest();
  xhr.withCredentials = true;
  var url = URL_PREFIX + '/rest/gce/instance/removal';
  xhr.open('POST', url, true);
  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4) {
          ...
          google.maps.event.addListener(
              marker, 'click', (function(marker, contentString, infowindow) {
                return function() {
                  ...
                  $.ajax({
                    type: 'GET',
                    url: URL_PREFIX + '/rest/cityTemperature?id=' + id +
                        '&numRecord=5',
                    async: true,
                    xhrFields: {withCredentials: true},
                    success: function(data) {
...
function loadChart(inputData, id) {
  ...
  intervals.push(setInterval(function() {
    $.ajax({
      type: 'GET',
      url: URL_PREFIX + '/rest/cityTemperature?id=' + id + '&numRecord=1',
      async: true,
      xhrFields: {withCredentials: true},
      success: function(data) {
        if (data.code != 200) {
